...
ALLOWED_EXTENSIONS = {'txt', 'pdf', 'docx', 'xlsx', 'xls', 'csv', 'pptx', 'ppt', 'md', 'vtt'}
...
def process_async_ai_summary(request_data):
    """Process AI summary asynchronously"""
    logger.info(f"Processing async AI summary: {request_data['summary_id']}")
...
"""
        ...
        if original_filename.endswith('.md'):
            ...
            new_filename = parts[0] + '_AI.' + (parts[1] if len(parts) > 1 else 'txt')
        
        # Save to S3
        new_key = '/'.join(s3_key.split('/')[:-1]) + '/' + new_filename
        
        # Convert metadata values to strings
        safe_metadata = {'original_document': s3_key, 'ai_summary': 'true'}
        ...
        # Save result
        user_prefix = get_user_s3_prefix(request_data['user_id'])
        result_data = {
            'success': True,
        ...
        result_key = f"ai-summaries/{user_prefix}/results/{request_data['summary_id']}.json"
        s3.put_object(
            Bucket=PROCESSED_BUCKET,
            Key=result_key,
        ...
        request_key = f"ai-summaries/{user_prefix}/requests/{request_data['summary_id']}.json"
        s3.put_object(
            Bucket=PROCESSED_BUCKET,
            Key=request_key,
        ...
        logger.info(f"Async AI summary completed: {request_data['summary_id']}")
        return {'statusCode': 200, 'body': json.dumps({'success': True})}
        
    except Exception as e:
        logger.error(f"Error processing async AI summary: {str(e)}")
...
def lambda_handler(event, context):
    ...
    """
    try:
        # Check if this is an async AI summary processing request
        if event.get('async_processing') and event.get('summary_request'):
            return process_async_ai_summary(event['summary_request'])
        ...
        if path == '/documents/upload' and method == 'POST':
            ...
            # Check if async parameter is set
            body = json.loads(event.get('body', '{}'))
            if body.get('async', True):  # Default to async for long-running summaries
                return handle_ai_summary_async(event, headers, context, user_context)
            else:
                return handle_ai_summary(event, headers, context, user_context)
            
        elif path.startswith('/documents/ai-summary-status/') and method == 'GET':
            logger.info(f"Handling AI summary status check for path: {path}")
            return handle_ai_summary_status(event, headers, context, user_context)
...
def handle_ai_summary_async(event, headers, context, user_context):
    """Handle POST /documents/ai-summary endpoint with async processing"""
    logger.info("=== handle_ai_summary_async called ===")
    logger.info(f"User context: {user_context}")
    try:
        # Parse request body
        try:
        ...
        # Store the summary request in S3 for processing
        request_data = {
        ...
        # Save request to S3 for tracking
        request_key = f"ai-summaries/{user_prefix}/requests/{summary_id}.json"
        s3.put_object(
            Bucket=PROCESSED_BUCKET,
            Key=request_key,
            Body=json.dumps(request_data),
        ...
        # Start async processing (invoke same Lambda asynchronously)
        lambda_client = boto3.client('lambda')
        
        # Prepare payload for async processing
        async_payload = {
            'async_processing': True,
            'summary_request': request_data
        }
        ...
        try:
            ...
            logger.info(f"Started async AI summary processing: {summary_id}")
        except Exception as e:
            ...
            return {
                'statusCode': 500,
                'headers': headers,
                'body': json.dumps({'error': 'Failed to start async processing'})
            }
        ...
        return {
            ...
            'body': json.dumps({
                ...
                'summary_id': summary_id,
                'status': 'processing',
                'status_url': f'/documents/ai-summary-status/{summary_id}'
            })
        }
        
    except Exception as e:
        logger.error(f"Error in async AI summary: {str(e)}")
        return {
            'statusCode': 500,
            'headers': headers,
            'body': json.dumps({'error': 'Failed to start AI summary generation'})
        }

def handle_ai_summary_status(event, headers, context, user_context):
    """Handle GET /documents/ai-summary-status/{id} endpoint"""
    try:
        # Extract summary ID from path
        path = event.get('path', '')
        if '/ai-summary-status/' in path:
            summary_id = path.split('/ai-summary-status/')[-1]
        else:
            return {
                'statusCode': 400,
                'headers': headers,
                'body': json.dumps({'error': 'Summary ID required'})
            }
        ...
        try:
            obj = s3.get_object(Bucket=PROCESSED_BUCKET, Key=request_key)
            request_data = json.loads(obj['Body'].read().decode('utf-8'))
        except s3.exceptions.NoSuchKey:
            return {
                'statusCode': 404,
                'headers': headers,
                'body': json.dumps({'error': 'Summary request not found'})
            }
        except Exception as e:
        ...
        if request_data['status'] == 'completed':
            # Get the result
            result_key = f"ai-summaries/{user_prefix}/results/{summary_id}.json"
            try:
                result_obj = s3.get_object(Bucket=PROCESSED_BUCKET, Key=result_key)
                ...
                return {
                    'statusCode': 500,
                    'headers': headers,
                    'body': json.dumps({'error': 'Failed to get summary result'})
                }
        ...
        return {
            'statusCode': 500,
            'headers': headers,
            'body': json.dumps({'error': 'Failed to check summary status'})
        }
